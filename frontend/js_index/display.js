...
function renderProgress(data){
    if (!(data && data.iterations && data.iterations.length > 0)) {
      console.warn('No iteration data available to render progress.');
      return;
    }
    ...
    let accuracyData, fairnessScore, accuracyScore;
    ...
    if (currentSelectedMetric === 'Max_Epsilon') {
      ...
      leftChartData = data.iterations.map((item) => {
        let value = 0;
        if (item.metrics && item.metrics[currentSelectedMetric] !== undefined) {
          const metricValue = item.metrics[currentSelectedMetric];
          ...
          console.warn(`Metric ${currentSelectedMetric} missing in iteration ${item.iteration}, defaulting to 0`);
        }
        return { iteration: item.iteration, value: value };
      });
    ...
    accuracyData = data.iterations.map((item) => ({
      iteration: item.iteration,
      value: item && item.metrics && item.metrics.ACC ? item.metrics.ACC : 0
    }));

    console.log('[DEBUG] Accuracy data extracted (excluding iteration 0):', accuracyData);
    
    if (latestHistory.metrics[currentSelectedMetric] !== undefined) {
        const metricValue = latestHistory.matrics[currentSelectedMetric];
        if (typeof metricValue === 'object' && metricValue !== null) {
          const values = Object.values(metricValue).filter(v => typeof v === 'number');
    ...
    accuracyScore = latestHistory.matrics ? (latestHistory.matrics.ACC || 0.8).toFixed(4) : '0.8000';
      ...
      <div style="margin-top: 16px;">
        ...
        <div class="history-panel-external">
          <div class="history-list-external">
            ${data.iterations.filter(item => item && item.metrics).map((item) => `
            <div class="history-item-external" onclick="showHistoryDetail(${item.iteration})">
              <span class="history-data">Iteration ${item.iteration}</span>
              ...
              <span class="history-divider">|</span>
              <span class="history-data">Fairness: ${item.metrics.Overall_Fairness !== undefined ? item.metrics.Overall_Fairness.toFixed(4) : 'N/A'}</span>
              <span class="history-divider">|</span>
              <span class="history-data">Acc: ${item.metrics.ACC !== undefined ? item.metrics.ACC.toFixed(4) : 'N/A'}</span>
            </div>
            `).join('')}
          </div>
        </div>
...
function showProcessStep(stepIndex) {
    ...
    const currentSelectedMetric = state.selectedMetric;
    console.log(`[DEBUG] Rendering chart for metric: ${currentSelectedMetric}, step: ${stepIndex}`);
    ...
    if (currentSelectedMetric === 'Max_Epsilon' && state.maxEpsilonSeries.length > 0) {
    // Max Epsilon 模式
    ...
    accuracyData = [];
    console.log('[DEBUG] No accuracy data - history.length:', state.history.length);
    }
